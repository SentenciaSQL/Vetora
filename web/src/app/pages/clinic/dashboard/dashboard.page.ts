  ...
  template: `
    @if (auth.isSuperAdmin()) {
      ...
      <div class="mt-6 flex flex-wrap gap-3">
        <a routerLink="/calendar" [queryParams]="{ book: 1 }" class="btn-primary">{{ 'calendar.request' | translate }}</a>
        <a routerLink="/pets" [queryParams]="{ register: 1 }" class="btn-secondary">{{ 'pets.register' | translate }}</a>
